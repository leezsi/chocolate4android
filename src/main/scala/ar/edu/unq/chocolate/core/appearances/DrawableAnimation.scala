...
class DrawableAnimation(val meanTime: Double, val sprites: DrawableSprite*) extends ComplexAppearance {
  var currentIndex = 0
  ...
  protected def doRenderAt(x: Double, y: Double, graphics: Canvas) = currentSprite.renderAt((x, y), graphics)
